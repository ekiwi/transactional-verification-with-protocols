...
from examples.multi0keep import spec as multi0keep_spec

toplevel = 'multi0keep2const'
src = ['multi0keep.v', 'multi0keep2const.v']
mod = Module.load(toplevel, src)

data_in = Symbol('multi0keep2const.Delay.dat_in', BVType(64))
data_out = Symbol('multi0keep2const.Delay.dat_out', BVType(64))
...
no_abstraction_check = VerificationProblem(
	spec=spec, implementation='multi0keep2const', invariances=invariances, mappings=[])

abstracted_check = VerificationProblem(
	spec=spec, implementation='multi0keep2const', invariances=[], mappings=[],
	submodules={'lsb_unit': multi0keep_spec, 'msb_unit': multi0keep_spec})
